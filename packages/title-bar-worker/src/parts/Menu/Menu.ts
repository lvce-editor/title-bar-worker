...
export const getIndexToFocusNextStartingAt = (items: readonly (MenuEntry | VisibleMenuItem)[], startIndex: number): number => {
...
export const getIndexToFocusFirst = (items: readonly (MenuEntry | VisibleMenuItem)[]): number => {
  return getIndexToFocusNextStartingAt(items, 0)
}

export const getIndexToFocusLast = (items: readonly (MenuEntry | VisibleMenuItem)[]): number => {
...
const getIndexToFocusPreviousStartingAt = (items: readonly (VisibleMenuItem | MenuEntry)[], startIndex: number): number => {
  for (let i = startIndex; i > startIndex - items.length; i--) {
...
const canBeFocused = (item: MenuEntry | VisibleMenuItem): boolean => {
  switch (item.flags) {
